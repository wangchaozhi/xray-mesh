...
The coordinator relay already fans valid discovery packets out to every active peer except the sender. The client-side `-discovery-overlay` flag installs host routes for those multicast destinations on `xrmesh0`, which makes packets that use normal system routing enter the mesh data plane.
